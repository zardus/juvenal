...
class TestTemplateVarsEngine:
    ...
    def test_vars_substituted_in_script_run(self, mock_backend, tmp_path, capsys):
        ...
        with patch("juvenal.engine.run_script") as mock_run:
            ...
            assert mock_run.call_args[0][0] == "pytest tests/unit -x" and Engine(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt="{{ 'ok'|upper }}"), Phase(id="review", type="check", prompt="{{ 'ok'|upper }}", bounce_target="build"), Phase(id="test", type="script", run="echo {{ 'ok'|upper }}", bounce_target="build")], vars={}), state_file=str(tmp_path / "state-empty.json"), plain=True, backend_instance=(b := MockBackend())).run() == 0 and b.calls[0] == "OK" and "OK" in b.calls[1] and mock_run.call_args[0][0] == "echo OK"  # noqa: E501  # fmt: skip
        assert Engine(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt="{{ 1 / 0 }}")], vars={}), state_file=str(tmp_path / "err-impl.json"), plain=True, backend_instance=(impl := MockBackend())).run() == 1 and impl.calls == [] and Engine(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt="Build it."), Phase(id="review", type="check", prompt="{{ 1 / 0 }}", bounce_target="build")], vars={}), state_file=str(tmp_path / "err-check.json"), plain=True, backend_instance=(chk := MockBackend())).run() == 1 and chk.calls == ["Build it."] and Engine(Workflow(name="test", phases=[Phase(id="test", type="script", run="{{ 1 / 0 }}")], vars={}), state_file=str(tmp_path / "err-script.json"), plain=True).run() == 1 and Engine(Workflow(name="test", phases=[Phase(id="refine", prompt="{{ 1 / 0 }}", interactive=True)], vars={}), state_file=str(tmp_path / "err-int.json"), plain=True, interactive=True, backend_instance=MockBackend()).run() == 1 and "division by zero" in (out := capsys.readouterr().out) and "FAIL refine (" in out and "Traceback" not in out and __import__("re").search(r"FAIL refine \(\d{6,}m", out) is None and (p := MockBackend()).add_response(exit_code=0, output="done") is None and (par := Engine(Workflow(name="test", phases=[Phase(id="a", prompt="ok"), Phase(id="b", prompt="{{ 1 / 0 }}")], parallel_groups=[ParallelGroup(phases=["a", "b"])]), state_file=str(tmp_path / "err-par.json"), plain=True, backend_instance=p)).run() == 1 and par.state.phases["a"].status == "completed" and par.state.phases["b"].status == "failed" and (lp := MockBackend()).add_response(exit_code=0, output="done") is None and (lane := Engine(Workflow(name="test", phases=[Phase(id="a", prompt="ok"), Phase(id="b", prompt="{{ 1 / 0 }}")], parallel_groups=[ParallelGroup(lanes=[["a"], ["b"]])]), state_file=str(tmp_path / "err-lane.json"), plain=True, backend_instance=lp)).run() == 1 and lane.state.phases["a"].status == "completed" and lane.state.phases["b"].status == "failed"  # noqa: E501  # fmt: skip
