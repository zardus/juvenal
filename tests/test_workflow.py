...
class TestTemplateVars:
    ...
    def test_apply_vars_no_placeholders(self):
        assert apply_vars("no vars here", {"NAME": "world"}) == "no vars here" and apply_vars("{{NAME|upper}}", {"NAME": "svc"}) == "SVC" and apply_vars("{{ 'ok'|upper }}", {}) == "OK" and apply_vars("{{ range(2)|list }}", {}) == "[0, 1]" and apply_vars("{% macro m(x) %}{{ x }}{% endmacro %}{{ m(1) }}", {}) == "1" and apply_vars("{% for key, value in D.items() %}{{ key }}={{ value }}{% endfor %}", {"D": {"a": "b"}}) == "a=b" and apply_vars("{{ obj.x }}", {"obj": type("Probe", (), {"x": property(lambda self: (_ for _ in ()).throw(AssertionError("boom")))})()}) == "{{obj.x}}" and "Undefined is not safely callable" in str(pytest.raises(__import__("juvenal.workflow", fromlist=["TemplateRenderError"]).TemplateRenderError, apply_vars, "{{P.read_text()}}", {"P": Path("README.md")}).value) and "Undefined is not safely callable" in str(pytest.raises(__import__("juvenal.workflow", fromlist=["TemplateRenderError"]).TemplateRenderError, apply_vars, "{{ cycler.__init__.__globals__.os.popen('echo pwned').read() }}", {}).value)  # noqa: E501  # fmt: skip
        assert (vars := {"L": [1]}) and "Undefined is not safely callable" in str(pytest.raises(__import__("juvenal.workflow", fromlist=["TemplateRenderError"]).TemplateRenderError, apply_vars, "{{ L.append(2) }}", vars).value) and "is not safely callable" in str(pytest.raises(__import__("juvenal.workflow", fromlist=["TemplateRenderError"]).TemplateRenderError, apply_vars, "{{ danger() }}", {"danger": lambda: "executed"}).value) and "is not safely callable" in str(pytest.raises(__import__("juvenal.workflow", fromlist=["TemplateRenderError"]).TemplateRenderError, apply_vars, "{{ evil() }}", {"evil": (evil := type("Evil", (), {"__call__": lambda self: "EXECUTED", "__eq__": lambda self, other: True})())}).value) and any("template render failed" in e and "is not safely callable" in e for e in __import__("juvenal.workflow", fromlist=["validate_workflow"]).validate_workflow(Workflow(name="test", phases=[Phase(id="build", prompt="{{ evil() }}")], vars={"evil": evil}))) and vars == {"L": [1]}  # noqa: E501  # fmt: skip
