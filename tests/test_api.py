...
def test_staged_plan_and_do_rejects_invalid_elif_branch_template(tmp_path):
    planned_yaml = """\
name: planned
vars:
  ok: false
phases:
  - id: execute
    prompt: "{% if ok %}A{% elif missing %}B{% endif %}"
"""
...
def test_staged_plan_and_do_rejects_invalid_jinja_syntax_template(tmp_path):
    planned_yaml = """\
name: planned
phases:
  - id: execute
...
"""
    ...
    assert "invalid Jinja2 prompt" in str(exc_info.value)
