...
class TestEnhancedDryRun:
    ...
    def test_dry_run_invalid_jinja_syntax_does_not_crash(self, tmp_path, capsys):
        """Dry-run reports Jinja syntax errors without raising a traceback."""
        workflow = Workflow(
            name="test",
            phases=[Phase(id="setup", type="implement", prompt="{{ PROJECT")],
        )
        engine = Engine(workflow, state_file=str(tmp_path / "state.json"), dry_run=True, plain=True)
        assert engine.run() == 1
        captured = capsys.readouterr()
    ...
    def test_dry_run_jinja_render_error_does_not_crash(self, tmp_path, capsys):
        """Dry-run reports Jinja render errors without raising a traceback."""
        workflow = Workflow(
            name="test",
            phases=[Phase(id="setup", type="implement", prompt="{{ 1 / 0 }}")],
        )
        engine = Engine(workflow, state_file=str(tmp_path / "state.json"), dry_run=True, plain=True)
        assert engine.run() == 1
        captured = capsys.readouterr()
