...
class TestTemplateVarValidation:
    ...
    def test_defined_var_passes(self):
        ...
        assert all(validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt=prompt)], vars={"PROJECT": "myapp"})) == [] for prompt in ("{% if X is defined and Y == 'a' %}ok{% endif %}", "{% if X|default('a') == 'a' %}ok{% endif %}", "{% raw %}{{LITERAL}}{% endraw %}")) and validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt="{% for key, value in D.items() %}{{ key }}={{ value }}{% endfor %}")], vars={"D": {"a": "b"}})) == [] and all(any("Y" in e and "no value defined" in e for e in validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt=prompt)], vars={"X": "x"}))) for prompt in ("{% if X is defined and Y == 'a' %}ok{% endif %}", "{% if X|default('a') == Y %}ok{% endif %}")) and any("recursive data" in e for e in validate_workflow(Workflow(name="test", phases=[Phase(id="build", prompt="{{DATA}}")], vars={"DATA": (lambda d: d.update(self=d) or d)({})})))  # noqa: E501  # fmt: skip
