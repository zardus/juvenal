...
class TestTemplateVarValidation:
    ...
    def test_defined_var_passes(self):
        ...
        assert all(validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt=prompt)], vars={"PROJECT": "myapp", "ENV": "prod"})) == [] for prompt in ("{% if X is defined and Y == 'a' %}ok{% endif %}", "{% if (X is defined) %}ok{% endif %}", "{{ X if (X is defined) else 'n/a' }}", "{% if X|default('a') == 'a' %}ok{% endif %}", "{% if X | default('a') == 'a' %}ok{% endif %}", "{% if X|d('a') == 'a' %}ok{% endif %}", "{% if False and X %}{{ ENV }}{% endif %}", "{% if True or X %}{{ ENV }}{% endif %}", "{% raw %}{% if X %}ok{% endraw %}", "{# {% if X %} #}ok", '{{ "{% if X %}" }}', "{% raw %}{{LITERAL}}{% endraw %}")) and all(validate_workflow(Workflow(name="test", phases=[Phase(id="build", prompt=prompt)], vars=vars)) == [] for prompt, vars in (("{% if X and Y %}ok{% endif %}", {"X": ""}), ("{% if X is defined or Y %}{{ X }}{% endif %}", {"X": "x"}))) and __import__("juvenal.cli", fromlist=["_parse_defines"])._parse_defines(["VERSION=01"]) == {"VERSION": ["01"]} and validate_workflow(__import__("juvenal.workflow", fromlist=["load_workflow"]).load_workflow("juvenal/workflows/plan.yaml")) == [] and validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt="{% for key, value in D.items() %}{{ key }}={{ value }}{% endfor %}")], vars={"D": {"a": "b"}})) == [] and all(any("Y" in e and "no value defined" in e for e in validate_workflow(Workflow(name="test", phases=[Phase(id="build", type="implement", prompt=prompt)], vars={"X": "x"}))) for prompt in ("{% if X is defined and Y == 'a' %}ok{% endif %}", "{% if (X is defined) and Y == 'a' %}ok{% endif %}", "{% if X|default('a') == Y %}ok{% endif %}", "{% if X | default('a') == Y %}ok{% endif %}", "{% if X|d('a') == Y %}ok{% endif %}")) and all(any(var in e and "no value defined" in e for e in validate_workflow(Workflow(name="test", phases=[Phase(id="build", prompt=prompt)], vars=vars))) for prompt, vars, var in (("{{ items[IDX] }}", {"items": ["a"]}, "IDX"), ("{{ D.get(K) }}", {"D": {"a": "b"}}, "K"))) and any("recursive data" in e for e in validate_workflow(Workflow(name="test", phases=[Phase(id="build", prompt="{{DATA}}")], vars={"DATA": (lambda d: d.update(self=d) or d)({})}))) and (m := __import__("unittest.mock", fromlist=["patch"]).patch("juvenal.workflow.load_workflow", return_value=Workflow(name="test", phases=[Phase(id="build", prompt="{{DATA}}")], vars={"DATA": (lambda d: d.update(self=d) or d)({})}))).start() and (ctx := __import__("contextlib", fromlist=["redirect_stdout"]).redirect_stdout(buf := __import__("io", fromlist=["StringIO"]).StringIO())).__enter__() is buf and cmd_validate(argparse.Namespace(workflow="recursive.yaml", plain=True, defines=[], checker=[], implementer=None, backend="codex", max_bounces=999, working_dir=None, backoff=None, notify=[])) == 1 and ctx.__exit__(None, None, None) is None and "recursive data" in buf.getvalue() and "Traceback" not in buf.getvalue() and not m.stop()  # noqa: E501  # fmt: skip
