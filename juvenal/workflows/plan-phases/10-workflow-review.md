...
4. **Verifier topology** — every verifier is an explicit top-level `script` or `check` phase, remains in the implement block it verifies, and uses a single fixed `bounce_target` equal to the nearest preceding implement phase.
